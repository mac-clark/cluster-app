...
def load_data(file_path, zip_col, inactive_col=None):
    ...
    if file_path.endswith('.csv'):
        data = pd.read_csv(file_path, dtype={zip_col: str})  # Ensure ZIP codes retain leading zeros
...
def geocode_zip_codes(zip_codes, cache_file="geocode_cache.json"):
    ...
    new_cache = False
    total = len(zip_codes)  # Total ZIP codes for progress tracking

    if total == 0:  # Handle the case where there are no ZIP codes
        print("Progress: 1.0")
        return lat_lng

    for i, zip_code in enumerate(zip_codes):
        if zip_code in lat_lng:
            continue  # Skip already cached ZIP codes
        ...
        try:
            ...
            if location:
                ...
                print(f"Failed to geocode ZIP code: {zip_code}")
...
def cluster_lat_lng(lat_lng, radius_miles):
    ...
    # Progress output
    print("Progress: 75% (Clustering completed)")
...
def summarize_clusters(zip_cluster_map, lat_lng):
    ...
    # Progress output
    print("Progress: 90% (Summary completed)")
